...
contract AerariumMasterChef is Ownable {
    ...
    // distribution percentages: a value of 1M = 100%
    // 13.37% percentage of pool rewards that goes to the treasury.
    uint256 public TREASURY_PERCENTAGE = 133700;

    // 86.63% percentage of pool rewards that goes to LP holders.
    uint256 public POOL_PERCENTAGE = 866300;
        ...
        returns (uint256 pending)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            // a value of 1M = 100%
            uint256 aeraRewardsForPool = (aeraRewards * POOL_PERCENTAGE) / 1e6;
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0) {
                ...
                aera.transfer(
                    treasuryAddress,
                    (aeraRewards * TREASURY_PERCENTAGE) / 1e6
                );
